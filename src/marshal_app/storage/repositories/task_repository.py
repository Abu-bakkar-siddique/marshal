...
class TaskRepository:
    ...
    def create_standalone(self, title: str, comments: str = "") -> Task:
        next_sort_order = self._next_sort_order_for_standalone()
        cursor = self._connection.execute(
            """
            INSERT INTO tasks (project_id, title, comments, sort_order)
            VALUES (NULL, ?, ?, ?)
            """,
            (title, comments, next_sort_order),
        )
        self._connection.commit()
        task_id = int(cursor.lastrowid)
        task = self.get(task_id)
        if task is None:
            raise RuntimeError("Failed to create standalone task")
        return task
    ...
    def list_standalone(self) -> list[Task]:
        rows = self._connection.execute(
            """
            SELECT id, project_id, section_id, title, comments, is_done, sort_order,
                   completed_at, created_at, updated_at
    ...
    def reorder_standalone_tasks(self, ordered_task_ids: list[int]) -> None:
        ...
        self._connection.executemany(
            ...
            SET sort_order = ?,
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND project_id IS NULL AND is_done = 0
            """,
            updates,
        )
        self._connection.commit()
    ...
    def _next_sort_order_for_standalone(self) -> int:
        row = self._connection.execute(
            """
            SELECT COALESCE(MAX(sort_order), 0) AS max_sort_order
            FROM tasks
